...
function OarBorder({ className }: { className: string }) {
    return (
        <div
            aria-hidden
            className={`pointer-events-none absolute inset-x-0 z-20 flex items-center px-2 ${className}`}
        >
            {}
...
export default function TripsMap({ markers }: { markers: TripMapMarker[] }) {
    ...
    return (
        ...
        >
            {/* Oar borders (Darius: like the old website's oar dividers) —
                composed from the old theme's own handle/paddle SVGs with a
                shaft between; the bottom oar is mirrored. */}
            <OarBorder className='top-1.5' />
            <OarBorder className='bottom-1.5 -scale-x-100' />
            <Map
                initialViewState={{
                    longitude: -110.1,
                    latitude: 39.2,
